...
class _DockerFrameReader:
    ...
    def __init__(self, raw: socket.SocketIO) -> None:
        self._raw = raw
...
class RealDockerClient:
    ...
    def container_stdin(self, container_id: str) -> io.TextIOBase:
        ...
        Note: docker-py's ``attach_socket`` internally upgrades the HTTP
        connection to a TCP socket and returns a ``socket.SocketIO`` wrapper
        (from ``sock.makefile("rb")``) — not a raw ``socket.socket``.
        ``SocketIO`` created with read-only mode cannot be written to, so we
        extract the underlying ``socket.socket`` via ``_sock`` for stdin.
        """
        container = self.container_get(container_id)
        sock = container.attach_socket(
            params={"stdin": 1, "stream": 1}
        )
        # Docker SDK may return a read-only socket.SocketIO from the HTTP
        # upgrade response. Extract the underlying socket for writing.
        if isinstance(sock, socket.SocketIO):
